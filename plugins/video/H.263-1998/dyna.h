...
#ifndef __H263PDYNA_H__
#define __H263PDYNA_H__ 1
...
# endif // UNICODE
      ...
      if (_hDLL == NULL) {
        fprintf(stderr, "H.263+ plugin: error loading %s", path);
        char * err = dlerror();
        if (err != NULL)
...
#endif // _WIN32
      if (_hDLL != NULL) fprintf(stderr, "H.263+ plugin: successfully loaded %s\n", path);
      return _hDLL != NULL;
    }
